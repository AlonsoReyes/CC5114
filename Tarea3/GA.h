...
class GA {
	...
	public:
		...
		vector<Network> selection(vector<Network> population);
		void reproduct(vector<Network> selected_indiv);
		vector<Network> select_parents(vector< pair<Network, double > > pop);
		int choose_random(vector< pair <Network, double> > individuals);
};
